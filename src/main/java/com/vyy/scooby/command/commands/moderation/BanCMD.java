...
public class BanCMD extends Command {
    public BanCMD() {
        super.commandName = "ban";
        super.help = "Use this command to ban someone from your guild. To use, mention a user to kick and provide an (optional) reason";
        super.userPermissions.add(Permission.BAN_MEMBERS);
        super.botPermissions.add(Permission.BAN_MEMBERS);
    }
    ...
    protected void handle(CommandContext commandContext) {
        ...
        if (mentionedUsers.isEmpty() && args.length == 1) {
            channel.sendMessage(EmbedUtils.getInstance()
                    .errorEmbedText(":no_entry_sign: Error", "Please enter a member to ban", member).build())
                    .delay(3, SECONDS)
        ...
        commandContext.getGuild().retrieveMemberById(id).queue((m) -> {
            if (!member.canInteract(m) || !selfUser.canInteract(m)) {
                channel.sendMessage(EmbedUtils.getInstance()
                        .errorEmbedText(":no_entry_sign: Error", "I am unable to ban this member due to a lack of permissions", m).build())
                        .delay(3, SECONDS)
                        .flatMap(Message::delete).queue();
                return;
            }
            ...
            m.ban(7, sb.toString()).queue();
            channel.sendMessage(EmbedUtils.getInstance()
                    .successEmbedText(":white_check_mark: Success", "`" + m.getUser().getAsTag() + "` has been banned", member)
